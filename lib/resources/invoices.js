...
var invoiceLines = require('./invoice_lines');
...
module.exports = Resource()
  ...
  .consume(co(function* (client, obj, log) {
    ...
    if (discount) {
      ...
      yield discounts._consume(client, discount);
    }

    if (obj.lines.length) {
      properties.line_ids = map(obj.lines, (l) => l.id).join(',');

      // Consume lines as invoice_line objects
      for (var line of obj.lines) {
        line.invoice_id = obj.id;
        yield invoiceLines._consume(client, line);
      }
    }

    if (obj.id === 'in_8TZYXF2iiThdZt') {
      log.fatal({ obj, properties }, 'Found missing invoice!');
    }
